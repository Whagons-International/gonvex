...
try {
  ...
  const unsubscribe = client.subscribeLiveQuery(refs.list, {}, (message) => {
    if (message.type === "query.result" && Array.isArray(message.result)) latestRows = message.result;
  });
  ...
  process.stdout.write(JSON.stringify({ artifactHash, reloadedArtifactHash, direct, delegated }) + "\n");
} finally {
  client.close();
}
